...
def box_corner(box, width, height):
    return "top-left" if box[0] < width * 0.5 and box[1] < height * 0.5 else "bottom-right"


def interpolate_box(previous_box, next_box, ratio, width, height):
    previous = np.array(previous_box, dtype=np.float32)
    following = np.array(next_box, dtype=np.float32)
    return clamp_box(tuple(previous * (1.0 - ratio) + following * ratio), width, height)


def plan_video_boxes(raw_boxes, width, height, max_gap=24):
    planned = [[box] if box is not None else [] for box in raw_boxes]
    detected_frames = sum(bool(boxes) for boxes in planned)
    filled_frames = 0
    detected_indices = [index for index, box in enumerate(raw_boxes) if box is not None]
    for index, boxes in enumerate(planned):
        if boxes or not detected_indices:
            continue
        previous_indices = [value for value in detected_indices if value < index]
        next_indices = [value for value in detected_indices if value > index]
        ...
        if previous_index is not None and next_index is not None:
            ...
            if gap <= max_gap:
                previous_box = raw_boxes[previous_index]
                next_box = raw_boxes[next_index]
                previous_corner = box_corner(previous_box, width, height)
                next_corner = box_corner(next_box, width, height)
                if previous_corner == next_corner:
                    ratio = (index - previous_index) / max(next_index - previous_index, 1)
                    planned[index] = [interpolate_box(previous_box, next_box, ratio, width, height)]
                elif index - previous_index <= (gap + 1) / 2:
                    planned[index] = [previous_box]
                else:
                    planned[index] = [next_box]
                filled_frames += 1
        elif previous_index is not None and index - previous_index <= 3:
            planned[index] = [raw_boxes[previous_index]]
            filled_frames += 1
        elif next_index is not None and next_index - index <= 3:
            planned[index] = [raw_boxes[next_index]]
            filled_frames += 1
    return planned, detected_frames, filled_frames
...
def video_text_mask(frame, box):
    ...
    mask = ((gray > 88) & (hsv[:, :, 1] < 170)).astype(np.uint8) * 255
...
def clean_video():
    ...
    with tempfile.TemporaryDirectory(prefix="doubao-v3-") as temp_dir:
        ...
        while True:
            ...
            if full_rect:
                ...
                for box in planned_boxes[frame_index]:
                    current_mask = video_text_mask(frame, box)
        ...
        response.headers["X-Doubao-Planned-Frames"] = str(sum(bool(boxes) for boxes in planned_boxes))
        response.headers["X-Doubao-Mask-Pixels"] = str(mask_pixels)
        response.headers["X-Doubao-Mode"] = "temporal-gap-fill-inpaint"
        return response
